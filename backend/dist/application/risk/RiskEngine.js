...
class RiskEngine {
    ...
    async calculateForUser(userId) {
        ...
        const access = (await this.deps.userAccess.listByUserId(userId)).filter((a) => a.status === UserAccess_1.UserAccessStatuses.GRANTED);
        const permissions = await this.deps.permissions.listByIds(access.map((a) => a.permissionId));
        const permById = new Map(permissions.map((p) => [p.id, p]));
        const template = await this.deps.roleTemplates.findByRoleAndDepartment(user.role, user.departmentId);
        ...
        const peerPermissionCounts = new Map();
        for (const peer of peers) {
            const peerAccess = (await this.deps.userAccess.listByUserId(peer.id)).filter((a) => a.status === UserAccess_1.UserAccessStatuses.GRANTED);
        ...
        for (const a of access) {
            ...
            if (p.privilegeLevel >= 4) {
                score += 20;
                factors.privilege.push({ permissionId: p.id, added: 20, privilegeLevel: p.privilegeLevel });
            }
            else if (p.privilegeLevel >= 3) {
                score += 10;
                factors.privilege.push({ permissionId: p.id, added: 10, privilegeLevel: p.privilegeLevel });
            }
            // sensitive system risk
            if (p.isSensitiveSystem) {
                score += 15;
                factors.sensitive.push({ permissionId: p.id, added: 15, system: p.system });
            }
            // dormant permissions risk (last_used > 90 days or never used)
            const daysSinceUsed = a.lastUsedAt ? (0, dateUtils_1.differenceInDays)(now, a.lastUsedAt) : null;
            if (daysSinceUsed === null || daysSinceUsed > 90) {
                score += 10;
                factors.dormant.push({
                    permissionId: p.id,
                    added: 10,
                    daysSinceUsed,
                });
            ...
            if (!roleDefault.has(p.id)) {
                score += 12;
                factors.roleMismatch.push({ permissionId: p.id, added: 12 });
            }
